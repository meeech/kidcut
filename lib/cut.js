...
cut.start = function(input, output_dir, cut_file) {
  cli.ok('> Start', input, output_dir, cut_file);
  ...
  emitter.addListener(MERGE_EVENT, function() {
    ...
    merge(film_name, output_dir, time_codes);
  });
...
function generateClip(filename, output_dir, time_codes){
  ...
  output_file_name = _generate_clip_path(output_dir, time);
...
function merge(film_name, output_dir, time_codes) {
  ...
  for (i = 0; i < time_codes.length; i++) {
    input_file = _generate_clip_path(output_dir, time_codes[i]);

    //Need to initialize with an input_file
    if(!video) {
...
function _generate_clip_path(output_dir, time) {
  return output_dir.concat(time.start, '_', time.end, '.mp4');
}
